from srl_il.dataset.dataset_base import TrajectoryDataset, get_train_val_test_seq_datasets
...
class TestDataset(TrajectoryDataset):
    """
    ...
    """
    def __init__(self, data_directory: str, cache_data_size=1e6):
        """
        ...
        for f in glob(f'{data_directory}/**/*.h5', recursive=True):
            with h5py.File(f, "r", swmr=True, libver='latest') as h5_file:
                # check what datasets are there in this file
                try:
                    traj_data = {
                        key: (h5_file[key], f, h5_file[key].name) for key in ["actions_hand"] if key in h5_file
                    }
...
class test_train_robomimic:
    def __init__(self,          
        ...
        self.sequence_dataset = TestDataset(data_directory)
...
if __name__ == "__main__":
    datadir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../data_12_02_2024/processed"))
    print(datadir)
    td = TestDataset(datadir)
    print("Done")
